...
st.set_page_config(page_title="Nail Salon", page_icon="💅", layout="centered")
...
EXCEL_PATH = r"C:\Users\surff\OneDrive\デスクトップ\売上げver.2.0.xlsx"
SHEET_NAME = "入力シート"
CSV_PATH = os.path.join(os.path.dirname(__file__), "uriage_data.csv")
...
def save_row(row_data: dict):
    ...
    new_row["日付"] = str(pd.Timestamp(d).date())
    ...
    new_row["HPB"] = row_data["HPB"] if row_data["HPB"] > 0 else None
    new_row["割引"] = row_data["割引"] if row_data["割引"] > 0 else None
    hpb = row_data["HPB"] or 0
    discount = row_data["割引"] or 0
    new_row["請求額"] = row_data["金額"] - hpb - discount
    new_row["最終売上"] = row_data["金額"]
    new_row["備考"] = row_data["備考"] or None
    new_row["ちえり"] = 0
...
with tab1:
    ...
    # セッションステートで割引額を管理（フォーム外）
    if "discount_val" not in st.session_state:
        st.session_state.discount_val = 0
    if "amount_val" not in st.session_state:
        st.session_state.amount_val = 0
    ...
    with st.form("sales_form", clear_on_submit=True):
        ...
        amount = st.number_input("💴 金額（円）", min_value=0, step=100, value=0, key="amount_input")
        ...
        # 割引ボタン（フォーム内ではst.form_submit_buttonしか使えないのでradioで代用）
        ...
        manual_discount = st.number_input("割引金額（手入力）", min_value=0, step=100, value=0,
                                          disabled=(discount_type != "手入力"))
        ...
        elif discount_type == "30%":
            discount = int(amount * 0.30)
        elif discount_type == "手入力":
...
with tab2:
    ...
    try:
        ...
        if selected_year - 1 in years:
            df_prev = df[df["年"] == selected_year - 1]
            prev_total = df_prev["最終売上"].sum()
...
with tab3:
    ...
    try:
        ...
        fig2 = px.pie(
            menu_plot, names="メニュー", values="最終売上",
            title="メニュー別売上構成",
            color_discrete_sequence=["#1a1a1a", "#555", "#888", "#aaa", "#ccc", "#e0e0e0"],
            hole=0.4,
        )
        fig2.update_layout(
            height=380,
